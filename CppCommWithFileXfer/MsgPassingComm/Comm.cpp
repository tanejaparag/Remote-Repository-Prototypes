...
// ver 2.3                                                         //
// Author - Parag Taneja , CSE 687 - OOD , Spring 2018
...
* ---------------
* Coom.h , Logger.h
* Utilities.h , Cpp11-BlockingQueue.h
* Filesystem.h

* This package has 1 class
* - ClientHandler -  This is ClientHandler for receiving messages and posting to the receive queue.
*Functions
* Receiver::Receiver(EndPoint ep, const std::string& name) - constructor sets por
* BlockingQueue<Message>* Receiver::queue() - returns reference to receive queue
* void Receiver::start(CallableObject& co) - starts listener thread running callable object
* void Receiver::stop() - tops listener thread
...
* void setQueue(BlockingQueue<Message>* pQ) - set BlockingQueue
* std::string readMsg(Socket& socket) - frame message string by reading bytes from socket
* bool receiveFile(Message msg) - expects msg to contain file and contentLength attributes , expects to be connected to appropriate destination
* void operator()(Socket socket) - reads messages from socket and enQs in rcvQ
* void Comm::stop() - Stops comm
* void Comm::postMessage(Message msg) - post message to sender queue
* Message Comm::getMessage() - get message from receiver
* std::string Comm::name() - setting name

*Maintenance History:
*  --------------------
*  ver 2.3 : 8 April 2018
*  - Handled file transfer from client by adding more folders string
*  - Added one more condition in start for reconnection of client through same port after disconnect
*  ver 2.2 : 27 Mar 2018
...
const std::string saveFilePath = "../../../../ServerFiles";
const std::string sendFilePath = "../ServerFiles";
const std::string clientFilePath = "../../../../ClientFiles";
...
void Sender::start()
{
  std::function <void()> threadProc = [&]() {
    while (true)
    {
      ...
      if (msg.to().address != lastEP.address || msg.to().port != lastEP.port || msg.command() == "reply_connect")
      {
        connecter.shutDown();
        //connecter.close();
...
bool Sender::sendFile(Message msg)
{
  ...
  if (msg.containsKey("serversending"))
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	std::string fileSpec = saveFilePath + "/";
	if (msg.containsKey("clientsending"))
	{
		fileSpec = fileSpec + msg.value("nmspc") + "/";
		fileSpec = fileSpec + msg.value("file_name");
	}
	if (msg.containsKey("serversending"))
	{
		fileSpec = clientFilePath + "/" + filename_;
		std::cout << FileSystem::Path::getFullFileSpec(fileSpec);
	}
